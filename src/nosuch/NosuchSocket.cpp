...
DWORD NosuchSocket::Initialize()
{
	...
	if ( NosuchNetworkInit() ) {
		DEBUGPRINT(("Unable to initialize networking in NosuchSocket::Initialize!?"));
		return 1;
	}
	...
	if (err) {
		DEBUGPRINT(("NosuchSocket::Initialized failed in WSAStartup!? err=%d",err));
		return err;
	}
...
#ifdef _M_X64
		// For some reason, I can't get the 64-bit version to compile, when
		// specifying an AcceptCondition function.
		// If I forcibly cast it in order to get it to compile, 
		SOCKET h_Socket = WSAAccept(conn->h_Socket, (LPSOCKADDR)&k_Addr, &s32_Len, NULL, (DWORD_PTR)this);
#else
	...
	*pu16_Port_source   = conn->u16_Port_source;
	...
	if (k_Events.lNetworkEvents & FD_CONNECT)
	{
		if (k_Events.iErrorCode[FD_CONNECT_BIT])
		{
			...
			DEBUGPRINT(("NosuchSocket event FD_CONNECT error = %d\n",u32_Error));
			goto _Exit;
		}

		// Store the time when the last action was executed on the socket
		conn->s64_IdleSince = GetTickCount64();
		DEBUGPRINT(("NosuchSocket event FD_CONNECT good!\n"));
...
int CALLBACK NosuchSocket::AcceptCondition(LPWSABUF pk_CallerId, LPWSABUF pk_CallerData, LPQOS pk_SQOS, LPQOS pk_GQOS, 
...
BOOL NosuchSocket::cList::Remove(DWORD u32_Index)
{
	...
	if (mk_Data[u32_Index].s8_SendBuf) {
		...
		mk_Data[u32_Index].s8_SendBuf = NULL;
	}
	if (mk_Data[u32_Index].pi_RecvMem) {
		delete mk_Data[u32_Index].pi_RecvMem;
		mk_Data[u32_Index].pi_RecvMem = NULL;
	}
