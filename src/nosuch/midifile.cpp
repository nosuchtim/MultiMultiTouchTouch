 ...
 *
 * Original release - mid-1980's, Tim Thompson
...
int (*Mf_header)(int,int,int) = NULLFUNC;
int (*Mf_trackstart)() = NULLFUNC;
int (*Mf_trackend)() = NULLFUNC;
int (*Mf_noteon)(int,int,int) = NULLFUNC;
int (*Mf_noteoff)(int,int,int) = NULLFUNC;
int (*Mf_pressure)(int,int,int) = NULLFUNC;
int (*Mf_controller)(int,int,int) = NULLFUNC;
int (*Mf_pitchbend)(int,int,int) = NULLFUNC;
int (*Mf_program)(int,int) = NULLFUNC;
int (*Mf_chanpressure)(int,int) = NULLFUNC;
int (*Mf_sysex)(int,char*) = NULLFUNC;
int (*Mf_arbitrary)(int,char*) = NULLFUNC;
int (*Mf_metamisc)(int,int,char*) = NULLFUNC;
int (*Mf_seqnum)(int) = NULLFUNC;
int (*Mf_eot)() = NULLFUNC;
int (*Mf_smpte)(int,int,int,int,int) = NULLFUNC;
int (*Mf_tempo)(int) = NULLFUNC;
int (*Mf_timesig)(int,int,int,int) = NULLFUNC;
int (*Mf_keysig)(int,int) = NULLFUNC;
int (*Mf_seqspecific)(int,char*) = NULLFUNC;
int (*Mf_text)(int,int,char*) = NULLFUNC;

/* Functions to implement in order to write a MIDI file */
int (*Mf_putc)(int) = NULLFUNC;
int (*Mf_writetrack)(int) = NULLFUNC;
...
/* private stuff */
static std::string Mf_readerror;
...
mfread()	 	/* The only non-static function in this file. */
{
	...
	if ( Mf_getc == NULLFUNC ) {
		Mf_readerror = "mfread() called without setting Mf_getc"; 
		return Mf_readerror;
	}
...
readheader()		/* read a header chunk */
{
	...
	if ( readmt("MThd") == EOF ) {
		Mf_readerror = "Found EOF while looking for MThd";
		return false;
	}
...
readtrack()		 /* read a track chunk */
{
	...
	while ( Mf_toberead > 0 ) {
		...
		if ( sysexcontinue && c != 0xf7 ) {
			Mf_readerror = "didn't find expected continuation of a sysex";
			return false;
		}

		if ( (c & 0x80) == 0 ) {	 /* running status? */
			if ( status == 0 ) {
				Mf_readerror = "unexpected running status";
				return false;
			}
			running = 1;
		}
		...
		if ( needed ) {		/* ie. is it a channel message? */
			...
			if ( needed > 1 ) {
				c2 = egetc();
				if ( c2 == EOF ) {
					Mf_readerror = "Unexpected EOF in chanmessage?";
					return false;
				}
			}
...
mfwrite(int format, int ntracks, int division, FILE *fp) 
{
    ...
    if ( Mf_putc == NULLFUNC ) {
	    Mf_readerror = "mfmf_write() called without setting Mf_putc";
		return;
	}

    if ( Mf_writetrack == NULLFUNC ) {
	    Mf_readerror = "mfmf_write() called without setting Mf_mf_writetrack"; 
		return;
	}
...
printf("length = %d\n",(int) trklength);
 	...
 	if(fseek(fp,offset,0) < 0) {
	    Mf_readerror = "error seeking during final stage of write";
		return;
	}
 ...
 */
int 
mf_write_midi_event(unsigned long delta_time, unsigned int type, unsigned int chan, unsigned char* data, unsigned long size)
{
    unsigned int i;
    unsigned char c;
...
void
mf_write_tempo(unsigned long tempo)
{
    /* Write tempo */
...
#if 0
unsigned long 
mf_sec2ticks(int secs, unsigned int division, unsigned int tempo, float secs)
{    
 ...
 */
float 
mf_ticks2sec(int ticks, unsigned int division, unsigned long tempo)
{
...
eputc(unsigned char c)			
{
	...
	if((Mf_putc) == NULLFUNC) {
		Mf_readerror = "Mf_putc undefined";
		return -1;
	}
